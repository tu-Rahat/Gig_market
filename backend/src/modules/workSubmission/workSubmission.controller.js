    ...
    try {
        ...
        if (
            ...
            return res.status(403).json({
                message:
                    "Only the selected worker can submit completed work"
            });
        }
...
const getWorkerAssignments = async (
    req,
    res
) => {
    try {
    ...
    try {
        ...
        const responseSubmissions =
            await Promise.all(
                submissions.map(
                    async (submission) => {
                        return await buildSubmissionResponse(
                            submission
                        );
                    }
                )
...
const reviewCompletedWork = async (
    req,
    res
) => {
    try {
